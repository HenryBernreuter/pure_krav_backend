...
server <- function(input, output, session) {
  ...
  # Add Lead and open modal for scheduling
  observeEvent(input$submit_lead, {
    ...
    new_lead <- data.table(
      ...
      first_class = NA  # Placeholder for the class to be scheduled later
    )
    ...
    sheet_write(leads, ss = sheet_urls[3], sheet = "leads_data")
    ...
    # Show a modal dialog to schedule the first class
    showModal(modalDialog(
      title = "Schedule Your First Class",
      tabsetPanel(
        tabPanel("Adults", 
                 selectInput("adults_time_modal", "Select Class Time for Adults", choices = c(
                   "Monday 7:15pm", "Tuesday 9:45am", "Tuesday 6:15pm", 
                   "Thursday 9:45am", "Thursday 6:15pm", "Saturday 8:45am")),
                 actionButton("schedule_adult_modal", "Schedule Class")
        ),
        tabPanel("Kids", 
                 selectInput("kids_time_modal", "Select Class Time for Kids", choices = c(
                   "Monday 4:15pm", "Tuesday 11:00am", "Tuesday 5:15pm", 
                   "Wednesday 4:15pm", "Thursday 5:15pm", "Saturday 9:45am")),
                 actionButton("schedule_kids_modal", "Schedule Class")
        ),
        tabPanel("Teens", 
                 selectInput("teens_time_modal", "Select Class Time for Teens", choices = c(
                   "Monday 4:15pm", "Monday 7:15pm", 
                   "Tuesday 11:00am", "Tuesday 5:15pm", "Tuesday 6:15pm", 
                   "Wednesday 4:15pm", "Thursday 5:15pm", 
                   "Thursday 6:15pm", "Saturday 8:45am", "Saturday 9:45am")),
                 actionButton("schedule_teens_modal", "Schedule Class")
        ),
        tabPanel("Women Only", 
                 selectInput("women_time_modal", "Select Class Time for Women", choices = c(
                   "Monday 7:15pm", "Tuesday 9:45am", "Tuesday 6:15pm", 
                   "Thursday 9:45am", "Thursday 6:15pm", "Saturday 8:45am")),
                 actionButton("schedule_women_modal", "Schedule Class")
        )
      ),
      easyClose = TRUE,
      footer = NULL
  ...
  # Logic to schedule the class for each category
  observeEvent(input$schedule_adult_modal, {
    selected_class <- input$adults_time_modal
    update_first_class(selected_class)
  })
  
  observeEvent(input$schedule_kids_modal, {
    selected_class <- input$kids_time_modal
    update_first_class(selected_class)
  })
  
  observeEvent(input$schedule_teens_modal, {
    selected_class <- input$teens_time_modal
    update_first_class(selected_class)
  })
  
  observeEvent(input$schedule_women_modal, {
    selected_class <- input$women_time_modal
    update_first_class(selected_class)
  })
  
  # Function to update the first class in the Google Sheet
  update_first_class <- function(selected_class) {
    sheet_data <- as.data.table(read_sheet(sheet_urls[3]))
    
    # Assuming the new lead is the last one added
    sheet_data[nrow(sheet_data), first_class := selected_class]
    
    # Write the updated leads data back to the Google Sheet
    sheet_write(sheet_data, ss = sheet_urls[3], sheet = "leads_data")
    ...
    showModal(modalDialog(
      ...
      paste("Your first class is scheduled for:", selected_class),
      easyClose = TRUE,
      footer = NULL
    ))
  }
  ...
  observeEvent(input$save2, {
    if (!is.null(input$table2)) {
      ...
      # Get the column names and types from the Google Sheet
      sheet_columns <- colnames(sheet_data)
      sheet_types <- sapply(sheet_data, class)
      browser()
      # Extract the input data from RHandsontable
      #input_data <- as.data.table(hot_to_r(input$table2))
      
      input_data <- as.data.table(input$table2$data)
      input_data <- data.table::rbindlist(lapply(input_data, as.data.table), fill = TRUE)
      # Ensure that input_data has the same columns as sheet_data
      # if (!all(sheet_columns %in% colnames(input_data))) {
      #   missing_cols <- setdiff(sheet_columns, colnames(input_data))
      #   for (col in missing_cols) {
      #     input_data[, (col) := NA]  # Add missing columns with NA values
      #   }
      # }
      
      # Reorder the columns to match the Google Sheet
      #input_data <- input_data[, ..sheet_columns]
      
      # Coerce column types to match the sheet_data
      for (col in sheet_columns) {
        set(input_data, j = col, value = as(input_data[[col]], sheet_types[col]))
      }
      
      # Write the updated data back to the Google Sheet
      sheet_write(input_data, ss = sheet_urls[2], sheet = "Sheet1")
      
      showModal(modalDialog(
        title = "Success",
        "Waivers data has been successfully updated.",
        easyClose = TRUE,
        footer = NULL
      ))
    }
